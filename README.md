...
- 卡片右上角优先显示 `豆瓣 / TMDB` 两者中的低分
- 如果 `豆瓣` 和 `TMDB` 都没有分数，则回退显示 `IMDb`
...
- `item_enrich_timeout`: 详情补分超时秒数（超时自动降级为原始详情，避免空白页）
- `enable_list_result_cache`: 是否启用推荐分类服务端缓存
- `list_cache_ttl_seconds`: 推荐分类服务端缓存 TTL 秒数
- `prewarm_list_cache_on_startup`: MP 启动后是否后台预热推荐分类缓存
- `prewarm_list_methods_limit`: 启动预热的分类数量上限
...
- 豆瓣网页兜底对“近期无评分”的条目新增冷却机制；冷却期间优先回读本地缓存并跳过重复网页请求，进一步降低被风控概率。
- 新增网页兜底精细控制：可整体验证关闭、可限制仅详情页触发、可限制每日网页请求上限。
- 列表补分增加单条超时保护，避免某些外部评分链路慢/阻塞导致整行骨架屏长期不消失。
- 新增详情补分超时降级，某个条目补分失败时直接回退原始详情，避免详情页空白。
- 新增推荐分类服务端缓存与启动预热，按“分类方法 + 分页”键缓存，客户端请求优先命中缓存；缓存过期后仅刷新对应分类，属于增量更新。
...
- 新增“评分稳定性闸门”：同一动作支持 `min_vote_count` + `min_days_since_release` 参数，可拦截“上映不久且投票人数偏少”的条目，避免新片早期评分波动导致误订阅。
- 修复 `workflow/plugin/actions` 查询时的动作序列化兼容问题，“调用插件”节点可在界面稳定加载并选择该动作。
- 新增“工作流自动过滤（无需调用插件节点）”：在工作流“获取媒体数据”阶段自动应用关键词排除和评分稳定闸门，兼容旧版 MP 前端不渲染“调用插件”节点的情况。
- 工作流模组化增强：`调用插件 -> 过滤媒体关键词` 的 `action_params` 留空时，会自动使用插件配置里的默认关键词与稳定性闸门参数，普通用户无需手填 JSON。
...
- 如果没有拿到 `豆瓣 / TMDB`，会自动回退到 `IMDb`；再缺失时才使用 `Bangumi`。
...
为减少隐私模式/冷启动下的骨架屏等待，插件新增了服务端分类缓存：

- 缓存键：`分类方法 + 分页参数`
- 命中缓存：直接返回补分后的结果，前端更快出图
- 过期策略：按 `list_cache_ttl_seconds` 自动过期
- 增量刷新：只会刷新当前请求对应的分类键，不会整站重刷
- 启动预热：开启后，MP 启动时后台预热常用分类缓存（不阻塞主进程）
