...
- 放行条件：`vote_count > min_vote_count` 且 `上映天数 > min_days_since_release`。
- 若媒体没有上映日期：自动降级为仅判断 `vote_count > min_vote_count`。
- 不满足上述条件的媒体会在“添加订阅”前被剔除。
...
- 插件会向工作流执行历史追加一条 `媒体过滤结果`。
- 默认 `发送消息` 动作会把这条历史发到 TG，因此能直接看到过滤后媒体名称列表。
